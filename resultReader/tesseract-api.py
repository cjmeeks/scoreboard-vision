...
class Tesseract(object):
    ...
    def setup_lib(cls, lib_path=None):
        if cls._lib is not None:
            return
        if lib_path is None:
            lib_path = "C:\\Program Files\\Tesseract-OCR\\libtesseract-5"
        ...
        #     cls.TessBaseAPI, ctypes.c_char_p, ctypes.c_char_p)

        lib.TessBaseAPISetImage.restype = None
        # lib.TessBaseAPISetImage.argtypes = (s
    ...
    def __init__(self, language='eng_best', datapath=None, lib_path=None):
        ...
        if self._lib.TessBaseAPIInit3(self._api, datapath, 'eng'):
    ...
    def set_variable(self, key, val):
        self._check_setup()
        self._lib.TessBaseAPISetVariable(self._api, str(key), val)
...
def tesseract_process_image2(tess, frame_piece):
    ...
    height, width, depth = frame_piece.frame.shape
    tess.set_variable("tesseract_char_whitelist",
    ...
    tess.set_variable("image_default_resolution", "100")

    tess.set_image(frame_piece.frame, width, height, 1)
...
if __name__ == '__main__':
    img = cv2.imread(
        'C:\\Users\\cjmeeks\\dev\\scoreboard-vision\\images\\ICSTCF.png')
    height, width, depth = img.shape
    print(datetime.utcnow())
    tess = Tesseract()
    print("ocr image Start:{}".format(datetime.utcnow()))
    frame_piece = FramePiece(img, None)
